...
// ========================================
// RUTAS NUEVAS PARA DEBUG DE SOMBREROS
// ========================================

// 🔹 RUTA PARA VERIFICAR SOMBREROS DE USUARIO
...
// 🔹 RUTA PARA DEBUG DE SOMBREROS
app.get("/debug/hats", async (req, res) => {
  try {
    const users = await User.find({}, { username: 1, selectedHat: 1, ownedHats: 1 });
    
    const hatStats = users.map(user => ({
      username: user.username,
      selectedHat: user.selectedHat,
      ownedHats: user.ownedHats || [],
      ownedCount: (user.ownedHats || []).length
    }));

    res.json({
      totalUsers: users.length,
      usersWithHats: users.filter(u => u.selectedHat).length,
      hatStats: hatStats
    });
  } catch (error) {
    console.error("Error en debug de sombreros:", error);
    res.status(500).json({ error: "Error interno del servidor" });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`🎩 Sistema de sombreros con debug activado`);
